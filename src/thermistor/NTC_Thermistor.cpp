...
double NTC_Thermistor_ESP32::readVoltage() {
	double v_adc_mV = (double)analogReadMilliVolts(this->pin);
	double vref_mV = (double)this->vref_mv;
	double adcRes = (double)this->adcResolution;

	double adc_counts = (v_adc_mV / vref_mV) * adcRes;
	double adc_counts_inverted = adcRes - adc_counts;

	return adc_counts_inverted;
